...
public class TrajectoryAngleSolver {
    ...
    public static double newtonRaphsonSolver(double distanceFromWallMeters, double initVelocityMPS) {
        ...
        int i = 0;
        while(Math.abs( trajectoryByAngle(distanceFromWallMeters, initVelocityMPS, thetaDegrees) ) > 1e-2 && i < 10) {
            thetaDegrees = createTangentLineXIntercept(
                thetaDegrees,
                trajectoryByAngle(
    ...
    public static double trajectoryByAngle(double distanceFromWallMeters, double initVelocityMPS, double thetaRadians) {
        setCAndV(distanceFromWallMeters, initVelocityMPS);

        return heightFromShootMeters - yGoalMeters + c * Math.tan(thetaRadians) 
               - c * c * v * secantSquared(thetaRadians);
    }

    public static double derivativeOfTrajectoryByAngle(double distanceFromWallMeters, double initVelocityMPS, double thetaRadians) {
        setCAndV(distanceFromWallMeters, initVelocityMPS);

        return (Math.PI / 180) * ( c * secantSquared(thetaRadians) - 2 *
        c * c * v * secantSquared(thetaRadians) * Math.tan(thetaRadians) );
    }
